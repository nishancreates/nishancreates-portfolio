...
export function NightSkyBackground({ accentHue = 200 }: NightSkyProps) {
  ...
  useEffect(() => {
    ...
    const stars: { x: number; y: number; r: number; alpha: number; twinkle: number; ts: number; col: string; }[] = [];
    ...
    CONSTELLATIONS.forEach(c => {
      c.stars.forEach(s => {
        constStars.push({ x: s.x * W, y: s.y * H, r: 2 + Math.random() * 0.6, alpha: 0.95, twinkle: Math.random() * Math.PI * 2, ts: 0.012, col: "#e0eaff" });
      });
    });

    class Shooter {
      x: number; y: number; len: number; speed: number; a: number; alpha: number;
      constructor() {
      ...
      dead() { return this.alpha <= 0 || this.x > W + 50 || this.y > H + 50; }
    }
